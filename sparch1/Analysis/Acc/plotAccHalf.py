...
def plot_results(df, model_types_to_plot, save_dir, plotMax, layers_to_plot, neurons_to_plot,broader_linewidth):
    ...
    for model_type in model_types_to_plot:
        ...
        label = 'complexLIF' if label == 'LIFcomplexHalfReset' else label
        ...
        if not subset.empty:
            ...
            if plotMax:
                ...
                plt.annotate(f'{max_acc:.4f} ', xy=(max_acc_layer, max_acc), 
    ...
    for model_type in model_types_to_plot:
        ...
        if not subset.empty:
            ...
            if plotMax:
                ...
                plt.axhline(y=max_acc, color=color, linestyle='--', linewidth=0.8)
                plt.annotate(f'{max_acc:.4f} ', xy=(x_positions[subset.index.get_loc(max_acc_index)], max_acc), 
    ...
    plt.tight_layout()
    plt.savefig(save_dir+filename, format='svg')
    plt.close()
...
def generate_filename(model_types_to_plot):
    ...
    return f"plot_{'_'.join(sanitized_names)}.svg"
...
def main():
    ...
    model_types_to_plot = [ "LIFcomplexHalfReset","adLIF","LIF"]
        #"adLIF","RadLIF"]
        #"LIFcomplexHalfReset", "RLIFcomplex1MinAlpha" ]
