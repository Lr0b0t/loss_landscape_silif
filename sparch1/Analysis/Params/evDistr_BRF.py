...
model_path = '../../SHD_runs/exp/paper_models/shd_BRF_3lay512_/checkpoints/best_model.pth'
...
for i, trained_layer in enumerate(trained_net.snn):
    print(trained_layer)
    if isinstance(trained_layer, BRFLayer):
        
        alpha_im = trained_layer.alpha_im.detach().cpu().numpy()
        alpha_real_off = trained_layer.alpha_real_off.detach().cpu().numpy()
        dt = 0.004
        p_w = (-1 + np.sqrt(1-np.square(dt*alpha_im)))/dt

        b = p_w - alpha_real_off
        alpha_real = b
        ...
        plt.scatter(alpha_real, alpha_im, color='blue', marker='x', label='Oscillatory Dynamics', s=30, alpha = 0.9)
        ...
        plt.savefig(f"../../plots/SHD/EV/BRFEVs_layer{i}.png")
        plt.show()
